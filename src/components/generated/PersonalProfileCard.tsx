...
}> = ({ children, style, onClick, onHoverChange }) => (
  <div
    onClick={onClick}
    className={`dashed-card${onClick ? ' dashed-card--clickable' : ''}`}
    style={{
...
export const PersonalProfileCard = () => {
  ...
  return (
    <div style={{
      width: '100vw', height: '100vh', backgroundColor: 'rgba(24,24,24,1)',
      ...
      <div className="portrait-blocker">
        ...
        <p className="portrait-blocker__title">Rotate your device</p>
        {/* Sub: Outfit — matches card description text */}
        <p className="portrait-blocker__sub">This portfolio is best viewed in landscape mode</p>
      </div>
      {/* ── Grid layer (masked at edges, sits behind everything) ── */}
      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage: gridBg, backgroundSize: '40px 40px',
        WebkitMaskImage: 'linear-gradient(to right, transparent 0%, black 14%, black 86%, transparent 100%), linear-gradient(to bottom, transparent 0%, black 14%, black 86%, transparent 100%)',
        WebkitMaskComposite: 'source-in',
        maskImage: 'linear-gradient(to right, transparent 0%, black 14%, black 86%, transparent 100%), linear-gradient(to bottom, transparent 0%, black 14%, black 86%, transparent 100%)',
        maskComposite: 'intersect',
        pointerEvents: 'none',
        zIndex: 0,
      } as React.CSSProperties} />

      {/* ── Canvas scaler (sits above the grid, no mask) ── */}
      <div style={{
        position: 'absolute', inset: 0,
        ...
        <div style={{ flexShrink: 0, transform: `scale(${scale})`, transformOrigin: 'center center' }}>
          <div style={{ width: 1440, height: 1024, position: 'relative' }}>
            ...
            {(() => {
              const ow = viewportW / scale;
              const oh = viewportH / scale;
              return (
                <div style={{
                  position: 'absolute',
                  left: -(ow - 1440) / 2,
                  ...
                  zIndex: 50,
                  pointerEvents: 'none',
                  backgroundColor: cardHovered ? 'rgba(0,0,0,0.32)' : 'rgba(0,0,0,0)',
                  backdropFilter: cardHovered ? 'blur(3px)' : 'blur(0px)',
                  WebkitBackdropFilter: cardHovered ? 'blur(3px)' : 'blur(0px)',
          ...
          <header style={{ position: 'absolute', left: 77, top: 59, width: 1286, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ color: 'rgba(97,97,97,1)', fontSize: 18, fontFamily: '"Outfit",sans-serif', textTransform: 'uppercase', letterSpacing: 1 }}>
                Online &nbsp;|&nbsp; Sri Lanka &nbsp;|&nbsp; {fmt(time)}
              </span>
            </div>
            <span style={{ color: 'rgba(97,97,97,1)', fontSize: 18, fontFamily: '"Outfit",sans-serif', textTransform: 'uppercase', letterSpacing: 1 }}>
          ...
          {/* ── Hero: Name ── */}
          <h1 style={{
            width: 260, color: 'rgba(217,217,217,1)', fontSize: 64,
            fontFamily: '"Galada",cursive', fontWeight: 400, lineHeight: '68px',
            textAlign: 'center', position: 'absolute', left: 590, top: 275, margin: 0,
          }}>
            Thakshila<br />Bandara
          </h1>

          {/* Portrait */}
          <img
            src={profileImg}
            alt="Thakshila Bandara"
            style={{ width: 278, height: 278, position: 'absolute', left: 581, top: 400, objectFit: 'cover', borderRadius: 4 }}
          />

          {/* Role subtitle */}
          <p style={{
            width: 310, color: 'rgba(180,180,180,1)', fontSize: 22, fontFamily: '"Outfit",sans-serif',
            fontWeight: 500, lineHeight: '29px', textAlign: 'center',
            position: 'absolute', left: 565, top: 680, whiteSpace: 'pre-line', margin: 0,
          }}>
            {'Embedded Systems Engineer\n& IoT Developer\nFounder @ Nodamic'}
          </p>

          {/* ── Tagline card ── */}
          <Card onHoverChange={onCardHover} style={{ width: 367, height: 118, left: 171, top: 118, padding: '18px 21px', zIndex: 10 }}>
            <p className="sweep-text" style={{ fontSize: 26, fontFamily: '"Libre Baskerville",serif', fontStyle: 'italic', fontWeight: 700, lineHeight: '40px', margin: 0 }}>
              I enjoy turning ideas into practical products
            </p>
          </Card>

          {/* ── About card ── */}
          <Card onHoverChange={onCardHover} style={{ width: 358, height: 328, left: 71, top: 212, padding: '22px 21px' }}>
            <Label>About</Label>
            <p className="sweep-text" style={{ fontSize: 24, fontFamily: '"Libre Baskerville",serif', fontStyle: 'italic', fontWeight: 700, lineHeight: '40px', margin: 0 }}>
              I design and build embedded systems and IoT solutions that connect hardware, software and real world impact.
            </p>
          </Card>

          {/* ── Skills card ── */}
          <Card onHoverChange={onCardHover} style={{ width: 320, height: 338, left: 111, top: 558, padding: '18px 21px' }}>
            <Label>Skills</Label>
          ...
          {/* ── Project 1 ── */}
          <Card onHoverChange={onCardHover} style={{ width: 389, height: 148, left: 919, top: 133, padding: '17px 21px' }}>
          ...
          {/* ── Project 2 ── */}
          <Card onHoverChange={onCardHover} style={{ width: 389, height: 148, left: 981, top: 263, padding: '17px 21px' }}>
          ...
          {/* ── Project 3 ── */}
          <Card onHoverChange={onCardHover} style={{ width: 402, height: 148, left: 919, top: 413, padding: '17px 21px' }}>
          ...
          {/* ── Education card ── */}
          <Card onHoverChange={onCardHover} style={{ width: 359, height: 286, left: 1000, top: 580, padding: '18px 21px' }}>
          ...
          {/* ── Achievement card ── */}
          <Card onHoverChange={onCardHover} style={{ width: 389, height: 148, left: 331, top: 820, padding: '18px 21px' }}>
          ...
          {/* ── Decorative arrows (imported SVG assets) ── */}
          ...
          <div style={{ position: 'absolute', left: 580, top: 912, width: 800, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 22 }}>
            {/* GitHub */}
            <a href="https://github.com/thakshilabandara" target="_blank" rel="noreferrer" title="GitHub" style={{ color: 'rgba(97,97,97,1)', transition: 'color 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'rgba(180,180,180,1)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'rgba(97,97,97,1)')}>
              <svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.09-.745.083-.73.083-.73 1.205.085 1.84 1.237 1.84 1.237 1.07 1.834 2.807 1.304 3.492.997.108-.775.418-1.305.762-1.605-2.665-.3-5.467-1.332-5.467-5.93 0-1.31.468-2.38 1.235-3.22-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.3 1.23a11.5 11.5 0 0 1 3.003-.404c1.02.005 2.047.138 3.003.404 2.29-1.552 3.297-1.23 3.297-1.23.653 1.652.242 2.873.118 3.176.77.84 1.233 1.91 1.233 3.22 0 4.61-2.807 5.625-5.48 5.92.43.37.814 1.102.814 2.222 0 1.606-.015 2.896-.015 3.286 0 .32.216.694.825.577C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
              </svg>
            </a>
            {/* LinkedIn */}
            <a href="https://linkedin.com/in/thakshilabandara" target="_blank" rel="noreferrer" title="LinkedIn" style={{ color: 'rgba(97,97,97,1)', transition: 'color 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'rgba(180,180,180,1)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'rgba(97,97,97,1)')}>
              <svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor">
            ...
            {/* Facebook */}
            <a href="https://facebook.com/thakshilabandara" target="_blank" rel="noreferrer" title="Facebook" style={{ color: 'rgba(97,97,97,1)', transition: 'color 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'rgba(180,180,180,1)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'rgba(97,97,97,1)')}>
              <svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor">
            ...
            {/* Instagram */}
            <a href="https://instagram.com/thakshilabandara" target="_blank" rel="noreferrer" title="Instagram" style={{ color: 'rgba(97,97,97,1)', transition: 'color 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'rgba(180,180,180,1)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'rgba(97,97,97,1)')}>
              <svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor">
            ...
            {/* Connect via Email */}
            <button onClick={handleCopy} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, color: 'rgba(97,97,97,1)', transition: 'color 0.2s' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'rgba(180,180,180,1)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'rgba(97,97,97,1)')}>
              <span style={{ fontSize: 22, fontFamily: '"Outfit",sans-serif', fontWeight: 500, color: 'inherit' }}>
